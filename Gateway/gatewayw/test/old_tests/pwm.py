...
def generate_pwm(connection, frequency, duty_cycle, gpio_pin):
    ...
    print('PWM Signal is initialized on pin {} with frequency of {}Hz and duty cycle of {}%'
          .format(gpio_pin, frequency, int(args.duty_cycle * 100)))
    error = connection.hardware_PWM(gpio_pin, int(frequency), int(duty_cycle))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpio', '-g', type=int, help='set gpio pin to output pwm signal (default: 18)', default=18) # 18 -> BCM0 = Pin 12 on board
